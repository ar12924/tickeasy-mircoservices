  ...
  fetch(`/maven-tickeasy-v1/user/member/verify?token=${token}`)
    ...
    .then((data) => {
      if (data.successful) {
        ...
        msg.textContent = data.message || "您的帳號已成功啟用，即將跳轉到登入頁面。";
        setTimeout(() => {
          window.location.href = "/maven-tickeasy-v1/user/member/login.html?verified=true";
        }, 2000);
      } else {
        title.textContent = "驗證失敗";
        msg.textContent = data.message || "驗證失敗";
        setTimeout(() => {
          window.location.href = "/maven-tickeasy-v1/user/member/login.html?verified=false";
        }, 3000);
      }
    })
    .catch(() => {
