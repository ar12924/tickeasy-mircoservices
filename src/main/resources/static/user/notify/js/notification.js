...
function category_count() {
  fetch("/api/notify/notification-list", {
    method: `POST`,
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      memberId: Number(sessionStorage.getItem("memberId")) || 0,
    }),
  })
    .then((resp) => resp.json())
    .then((notifications) => {
...
function notification_loaded(category) {
  ...
  fetch("/api/notify/notification-list", {
    ...
    .then((notifications) => {
      ...
      if (category == 2) {
        var category_count_get =
          document.querySelector("span.eventMind_ntf").innerHTML;
      ...
      if (category == 3) {
        var category_count_get =
          document.querySelector("span.sold_ntf").innerHTML;
      ...
      if (category == 4) {
        var category_count_get =
          document.querySelector("span.swap_ntf").innerHTML;
      ...
      if (category == 5) {
        var category_count_get =
          document.querySelector("span.change_ntf").innerHTML;
      ...
      for (let notification of notifications) {
        if (category == 1) {
          ...
          notification_el.insertAdjacentHTML(
            "afterbegin",
            `
          ...
          const ntf_el = document.querySelector(
            '[data-ntf-id="' + notification.memberNotificationId + '"]'
          );
          ...
          ntf_el.addEventListener("click", function () {
            if (notification.isRead != 1) {
              fetch("/api/notify/notification-read", {
                method: `POST`,
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                  memberId: Number(sessionStorage.getItem("memberId")) || 0,
...
document.addEventListener("DOMContentLoaded", function () {
  //先判斷有無登入
  fetch("/api/notify/check-login")
    .then((response) => response.json())
    .then((isLoggedIn) => {
      if (!isLoggedIn) {
        window.location.href = "/user/member/login.html"; // 如果未登入，跳轉到登入頁
        console.log("未登入");
      } else {
        notification_el.innerHTML = "";
        category_count();
...
document.addEventListener("click", function (e) {
  ...
  if (deleteTarget) {
    ...
    fetch("/api/notify/notification-unvisible", {
      method: `POST`,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
      ...
      .then((body) => {
        if (body.success) {
          ...
          let ntf_tab_dataId_get = ntf_nav_get
            .querySelector(".ntf_tab")
...
document.querySelectorAll(".ntf_tab").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    // 移除目前的 active 樣式
    document
      .querySelectorAll(".ntf_tab")
      .forEach((btn) => btn.closest("div.ntf_nav").classList.remove("-on"));
    document
      .querySelectorAll(".tab_content")
      .forEach((tab) => tab.closest("div.ntf_nav").classList.remove("-on"));
...
async function notification_clear() {
  try {
    const resp = await fetch("/api/notify/notification-clear-all", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        memberId: Number(sessionStorage.getItem("memberId")) || 0,
